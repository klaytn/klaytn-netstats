...
Node.prototype.setGeo = function(ip)
{
	...
	if (this.geo !== null) {
		this.info.name = this.geo.country + "/" + this.geo.region + "/" + this.geo.city + "(" + ip + ")";
	}
}
...
Node.prototype.setBasicStats = function(stats, callback)
{
	if( !_.isUndefined(stats) )
	{
		...
		}) )
		{
			...
			this.stats.p2pPeers = stats.p2pPeers;
...
Node.prototype.isInactiveAndOld = function()
{
	...
	// if(this.stats.block.timestamp === 0 || this.stats.block.timestamp > MAX_INACTIVE_TIME) {
	// 	return true;
	// }
